...
{% docs stg_jaffle_shop__customer %}
...
{% docs customers %}
Customer mart with lifetime order activity. The grain is one row per customer, including customers who have never placed an order. Order metrics include the first order date, most recent order date, and total number of orders.
{% enddocs %}
